...
const nonVulnerableQuery4 = "SELECT EMP_ID, LAST_NAME FROM EMPLOYEE WHERE CITY = 'Seattle' ORDER BY EMP_ID;"

const nonVulnerableQuery5 = "CREATE OR REPLACE VIEW [ Product List] AS SELECT ProductID, ProductName, Category FROM Products WHERE Discontinued = No;"

const nonVulnerableQuery6 =  "SELECT * FROM employees WHERE first_name LIKE 'P%'";

const nonVulnerableQuery7 = "SELECT Orders.OrderID, Customers.CustomerName, Orders.OrderDate FROM Orders INNER JOIN Customers ON Orders.CustomerID=Customers.CustomerID;"

const nonVulnerableQuery8 = "SELECT * FROM Customers WHERE Country= 'Mexico';"
